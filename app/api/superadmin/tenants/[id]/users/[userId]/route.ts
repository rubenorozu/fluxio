...
export async function PATCH(
    ...
    try {
        ...
        console.log('[PATCH User] Session:', session ? { role: session.user.role, tenantId: session.user.tenantId } : 'No session');
        ...
        if (session.user.role !== 'SUPERUSER') {
            console.log('[PATCH User] Authorization failed: role is', session.user.role);
...
export async function DELETE(
    ...
    try {
        ...
        // Debug: Log session info
        console.log('[DELETE User] Session:', session ? { role: session.user.role, tenantId: session.user.tenantId } : 'No session');
        ...
        // Verificar que sea SUPERUSER (sin importar el tenant)
        if (session.user.role !== 'SUPERUSER') {
            console.log('[DELETE User] Authorization failed: role is', session.user.role);
            return NextResponse.json({ error: 'No autorizado - Se requiere rol SUPERUSER' }, { status: 403 });
        }
